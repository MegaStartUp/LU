...
public class Player : MonoBehaviour
{
  ...
  //  GameObject ray_line;
    ...
    {/*
        ...
        {
            Vector3 temp;
            temp.x = Input.mousePosition.x / Camera.main.pixelWidth * General_Processor.sector_width_count;
            temp.y = Input.mousePosition.y / Camera.main.pixelHeight * General_Processor.sector_high_count;
            temp.z = 5f;

            Ray ray = Camera.main.
            RaycastHit[] hit_arr = Physics.RaycastAll(ray);
            Debug.Log(ray.origin);

			foreach(RaycastHit hit in hit_arr)
            {
                Debug.Log(hit.transform.name);
				if(hit.transform.name=="Load")hit.transform.GetComponent<Sector>().Click_blink();
			}
        }*/
        player_transl();
